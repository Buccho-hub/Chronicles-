...
def api_trigger(agent_name: str):
    ...
    valid = set(AGENT_MAP.keys()) | {"COUNCIL", "ORACLE"}
    if name not in valid:
        return jsonify({
            "error": f"Unknown agent '{agent_name}'",
            "valid_agents": sorted(valid),
        }), 404
    ...
    return jsonify({"triggered": name, "status": "started in background"})
...
def api_whitespace():
    """
    Cluster brief 'whitespace' observations by tag.
    ?mode=clusters returns:
      [{tag, observation_count, agents: [...], observations: [{observation, created_at, brief_id}, ...]}]
    """
    db     = get_db()
    ...
    for b in briefs:
        ...
        for tag in tags:
            ...
            clusters[tag]["agents"].update(agents)
    ...
    for tag, c in clusters.items():
        ...
        c["observations"].sort(key=lambda o: o.get("created_at") or "", reverse=True)
        result.append(c)

    result.sort(key=lambda c: c["observation_count"], reverse=True)
...
def api_predictions():
    """
    Derive predictions from briefs that have a 'timeline' field.
    Returns: [{id, headline, timeline, evidence, confidence, status, created_at}]
    """
    db     = get_db()
    briefs = db.get_briefs(limit=100)
    ...
    predictions.sort(key=lambda p: p.get("created_at") or "", reverse=True)
    return jsonify(predictions)


@app.route("/api/brief/<brief_id>")
def api_brief_detail(brief_id: str):
    """
    Return a brief plus its provenance chain: source council session
    and the originating dispatch, where available.
    """
    db = get_db()
    brief = db.get_brief(brief_id) if hasattr(db, "get_brief") else None

    if not brief:
        # fall back: search recent briefs
        for b in db.get_briefs(limit=100):
    ...
    if session_id:
        for s in db.get_recent_sessions(limit=100):
            if s.get("id") == session_id:
                session = s
                break
        if session:
            dispatch_id = session.get("source_dispatch_id")
            if dispatch_id and hasattr(db, "get_dispatch"):
                dispatch = db.get_dispatch(dispatch_id)

    return jsonify({"brief": brief, "session": session, "dispatch": dispatch})
