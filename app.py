...
UPDATES:
  - CASSANDRA agent added to scheduler (every 6h, lower SIL threshold)
  - /api/whitespace         : whitespace registry + clusters
  - /api/threads            : active topic threads
  - /api/threads/<tag>      : full thread for a specific tag
  - /api/predictions        : prediction tracking list
  - /api/rejections         : oracle rejection audit
  - /api/brief/<id>         : single brief with full provenance chain
  - /api/dispatches/<id>    : single dispatch
  - /api/sessions/<id>      : single council session
"""
...
TERRITORY_GROUPS = {
    ...
    "systemic":    {"JOHN",           "AUGUSTINE", "CASSANDRA"},
}
...
DIVERGENT_PAIRS = [
    ...
    ("RUTH",           "AMOS"),
    ("CASSANDRA",      "DANIEL"),   # early signal vs established-pattern reading
]
...
HIGH_SIGNAL_KEYWORDS = [
    ...
    "tipping point", "inflection", "irreversible",
]
...
def start_scheduler():
    ...
    # CASSANDRA: every 6h, offset 3h from Hildegard; lower SIL threshold handled in agent
    sched.add_job(lambda: run_agent("CASSANDRA"),      "interval", hours=6, id="cassandra",minutes=180, jitter=60)
    ...
    sched.start()
    logger.info("Scheduler started (9 agents + council + oracle)")
    return sched
...
@app.route("/api/dispatches/<dispatch_id>")
def api_dispatch_single(dispatch_id: str):
    """Single dispatch fetch — used for provenance chain."""
    db = get_db()
...
def api_brief_single(brief_id: str):
    """Full brief with provenance chain (brief → session → dispatch → raw_data)."""
    db    = get_db()
    chain = db.get_brief_provenance(brief_id)
    if not chain:
        return jsonify({"error": "Not found"}), 404
...
def api_session_single(session_id: str):
    db = get_db()
...
def api_whitespace():
    """
    Whitespace Registry.
    ?mode=clusters  → grouped by tag, sorted by observation count
    ?mode=raw       → flat list of observations (default)
    """
    db   = get_db()
    mode = request.args.get("mode", "clusters")
    if mode == "clusters":
        return jsonify(db.get_whitespace_clusters())
    return jsonify(db.get_whitespace_registry(
...
def api_threads():
    """Active topic threads (tags with 2+ dispatches)."""
    db  = get_db()
    min_d = int(request.args.get("min", 2))
    return jsonify(db.get_active_threads(min_dispatches=min_d))


@app.route("/api/threads/<tag>")
def api_thread_detail(tag: str):
    """Full chronological dispatch chain for a specific tag."""
    db    = get_db()
    limit = min(int(request.args.get("limit", 20)), 50)
    return jsonify(db.get_topic_thread(tag, limit=limit))


@app.route("/api/predictions")
def api_predictions():
    """Prediction tracking — ORACLE timeline claims and their verification status."""
    db    = get_db()
    limit = min(int(request.args.get("limit", 30)), 100)
    return jsonify(db.get_predictions(limit=limit))


@app.route("/api/predictions/<prediction_id>/verify", methods=["POST"])
def api_verify_prediction(prediction_id: str):
    """Mark a prediction confirmed or refuted."""
    body     = request.get_json(force=True, silent=True) or {}
    status   = body.get("status", "confirmed")
    evidence = body.get("evidence", "")
    if status not in ("confirmed", "refuted"):
        return jsonify({"error": "status must be confirmed or refuted"}), 400
    get_db().verify_prediction(prediction_id, status, evidence)
    return jsonify({"verified": prediction_id, "status": status})


@app.route("/api/rejections")
def api_rejections():
    """Oracle rejection audit trail."""
    db    = get_db()
    limit = min(int(request.args.get("limit", 20)), 50)
    return jsonify(db.get_rejections(limit=limit))
