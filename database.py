...
class SQLiteDB:
    ...
    def _init(self):
        with self._conn() as c:
            ...
            # Migrate: add columns that may be missing on pre-existing tables
            existing_cols = {row[1] for row in c.execute("PRAGMA table_info(briefs)")}
            for col, coltype in (
                ("ancient_parallel", "TEXT"),
                ("whitespace",       "TEXT"),
                ("timeline",         "TEXT"),
                ...
                if col not in existing_cols:
                    try:
                        c.execute(f"ALTER TABLE briefs ADD COLUMN {col} {coltype}")
                    except Exception as e:
                        logger.warning("SQLite migration warning (%s): %s", col, e)
            ...
            c.execute("""
                INSERT OR REPLACE INTO briefs
                  (id, source_session_id, headline, verdict, ancient_parallel,
                   evidence, implications, whitespace, timeline, action_items,
                   confidence, tier, agents, tags, created_at, published)
                VALUES
                  (:id,:source_session_id,:headline,:verdict,:ancient_parallel,
                   :evidence,:implications,:whitespace,:timeline,:action_items,
                   :confidence,:tier,:agents,:tags,:created_at,:published)
...
class SupabaseDB:
    ...
    def _init(self):
        ...
        # Migrate: add columns that may be missing on pre-existing tables
        migration_cols = [
            ("briefs", "ancient_parallel", "TEXT"),
            ("briefs", "whitespace",       "TEXT"),
            ("briefs", "timeline",         "TEXT"),
        ]
        conn = self._conn()
        try:
            with conn.cursor() as cur:
                for table, col, coltype in migration_cols:
                    try:
                        ...
                        logger.warning("Migration warning (%s.%s): %s", table, col, e)
            conn.commit()
        except Exception as e:
            conn.rollback()
            logger.warning("Migration block error: %s", e)
        finally:
            conn.close()
        ...
        try:
            with conn.cursor() as cur:
                ...
                """, (
                    ...
                    b.get("verdict"), b.get("ancient_parallel"),
                    b.get("evidence","[]"),
                    b.get("implications"), b.get("whitespace"), b.get("timeline"),
                    b.get("action_items","[]"),
